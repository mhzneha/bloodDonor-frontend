...
export default function NotificationsScreen() {
  ...
  const handlePress = async (item: AppNotification) => {
    ...
    if (item.notifiable_type === "BloodDonationRequest") {
      // someone requested blood from this donor
      router.push({
        pathname: "/blood-donor/request",
        params: { id: String(item.notifiable_id) },
      });
    } else if (item.notifiable_type === "BloodRequest") {
      // this donor's own blood request got matched/accepted
      router.push({
        pathname: "/(tabs)/blood-request/matching-donor", // swap for your actual request-detail route
        params: { id: String(item.notifiable_id) },
      });
    }
  };
  ...
  return (
    <View className="flex-1 bg-white">
      <View className="flex-row items-center justify-between px-5 pt-10 pb-4">
        <Text className="text-xl font-bold text-black-300">Notifications</Text>
        {unreadCount > 0 && (
